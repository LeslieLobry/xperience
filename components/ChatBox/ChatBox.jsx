...
const NotificationAppelEntrant = dynamic(() => import("../NotificationAppelEntrant/NotificationAppelEntrant"), { ssr:
false });
...
if (!seconds || isNaN(seconds) || !isFinite(seconds) || seconds < 0) return "0:00" ; const m=Math.floor(seconds / 60);
  const s=Math.floor(seconds % 60); return `${m}:${s < 10 ? "0" + s : s}`; } // Listen notifications d'appel entrant
  (Ably) useEffect(()=> {
  ...
  console.log("Appel lancé, notifications envoyées à :", participantsAutres.filter((p) => p.id !== utilisateur.id).map(p
  => p.id));
  ...
  if (!seconds || isNaN(seconds) || !isFinite(seconds) || seconds < 0) { resolve("0:00"); } else { const
    m=Math.floor(seconds / 60); const s=Math.floor(seconds % 60); const dureeStr=`${m}:${s < 10 ? "0" + s : s}`;
    console.log("[AUDIO] HACKED duration:", seconds, dureeStr); resolve(dureeStr); } URL.revokeObjectURL(audio.src);
    resolved=true; }; } else { let seconds=audio.duration; if (!seconds || isNaN(seconds) || !isFinite(seconds) ||
    seconds < 0) { resolve("0:00"); } else { const m=Math.floor(seconds / 60); const s=Math.floor(seconds % 60); const
    dureeStr=`${m}:${s < 10 ? "0" + s : s}`; console.log("[AUDIO] duration:", seconds, dureeStr); resolve(dureeStr); }
    URL.revokeObjectURL(audio.src); resolved=true; } }; setTimeout(()=> {
    if (!resolved) {
    resolve("0:00");
    ...
    <div className="chatbox-container">
      <ChatHeader participants={participantsAutres} inCall={inCall} onCallAudio={()=> startCall(false)}
        ...
        <MessagesList messages={messages} utilisateur={utilisateur} onReact={handleReaction} lastReads={lastReads}
          typingPseudo={isTyping ? typingPseudo : null} hasMore={hasMore} onLoadMore={loadMoreMessages}
          onDelete={handleDelete} prenomsCouple={prenomsCouple} />
        ...
        <ChatInput utilisateur={utilisateur} conversationId={conversationId} texte={texte} setTexte={setTexte}
          showEmojiPicker={showEmojiPicker} setShowEmojiPicker={setShowEmojiPicker} onMessageSent={async (contenu,
          type="TEXTE" , membreParlant, isImage=false)=> {
          if (isImage) {
          ...
          <NotificationAppelEntrant appel={appelEntrant && appelEntrant.from?.id !==utilisateur.id ? appelEntrant :
            null} onAccepter={(type)=> {
            clearTimeout(appelTimeoutRef.current);
            setAppelEntrant(null);
            sonnerieRef.current?.pause();
            sonnerieRef.current.currentTime = 0;
