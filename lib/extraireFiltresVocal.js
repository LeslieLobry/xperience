...
function extraireFiltresVocal(texte) {
  ...
  // Statut (NE PAS appliquer "tout le monde" si autour de moi)
  if (texte.includes("en ligne")) filtres.statut = "en_ligne";
  else if (
    (texte.includes("tous") || texte.includes("tout le monde")) &&
    !filtres.autourDeMoi // on ne met "all" que si ce n'est PAS autour de moi
  ) filtres.statut = "all";
  ...
  // Correction : "autour de moi" efface tout le reste incompatible
  if (filtres.autourDeMoi) {
    delete filtres.localisation;
    delete filtres.rayon; // enlève le rayon si tu veux être 100% strict, ou commente cette ligne si tu veux garder le rayon autour de moi
