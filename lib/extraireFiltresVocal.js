...
// 🚫 mots à ne jamais considérer comme une ville
const INVALID_LOCALISATIONS = [
  "plus de",
  "moins de",
  "plus",
  "moins",
...
function extraireFiltresVocal(inputTexte) {
  ...
  const autourDeMoi =
    /\b(autour de moi|pres de moi|pres de moi|a cote de moi|a cote de moi|ou je suis|ou je suis|ici)\b/.test(
      texte,
    );
  ...
  const distVille1 = texte.match(
    /(?:dans\s*(?:un\s*)?rayon\s*de|a|a)\s*(\d{1,3})\s*(?:km|kilometres?|kilometres?)\s*(?:de|autour de|pres de|pres de)?\s*([a-zàâäéèêëïîôöùûüç \-]{2,})/i,
  );
  if (distVille1) {
    rayon = distVille1[1];
  ...
  if (!ville && autourDeMoi) {
    const r = texte.match(
      /(?:rayon\s*de|a|a)\s*(\d{1,3})\s*(?:km|kilometres?|kilometres?)/i,
    );
    if (r) rayon = r[1];
  }
  ...
  if (!ville) {
    const villePrep = texte.match(
      /\b(?:a|a|sur|dans|vers)\s+([a-zàâäéèêëïîôöùûüç \-]{2,})\b/i,
    );
    if (villePrep) {
  ...
  if (!ville) {
    const villeProx = texte.match(
      /\b(?:pres de|pres de|autour de|a c[oô]te de|a cote de)\s+([a-zàâäéèêëïîôöùûüç \-]{2,})\b/i,
    );
  ...
  if (!ville) {
    const fallback = texte.match(
      /\b(?:de|a|a)\s+([a-zàâäéèêëïîôöùûüç \-]{2,})\b/i,
    );
    if (fallback) {
  ...
  if (ville) {
    const vnorm = normalizeToDb(ville).trim();

    // 🔒 on ignore les "fausses villes" type "plus de", "moins de", etc.
    if (!INVALID_LOCALISATIONS.includes(vnorm)) {
      // on garde la version normalisée (sans accents) dans les filtres
      filtres.localisation = vnorm;
  ...
  else if (/\b(tous|tout le monde)\b/.test(texte) && !filtres.autourDeMoi)
    filtres.statut = "all";
