...
export const runtime = "nodejs"; // important pour l'env et le SDK Resend
...
function hasValidAuth(req) {
  ...
  // 3) En option, reconnaître l’appel Vercel (indicatif)
  const isVercelCron = req.headers.get("x-vercel-cron") === "1";

  return (
    safeEqual(bearer, secret) ||
    safeEqual(q, secret) ||
    (isVercelCron && safeEqual(q, secret))
  );
}
...
export async function GET(req) {
  ...
  return NextResponse.json({ ok: true, hint: "POST with Bearer CRON_SECRET or ?token=CRON_SECRET" });
}
