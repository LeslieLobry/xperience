...
async function run(req) {
  ...
  try {
      ...
      include: {
        ...
        visite: {
          select: {
            ...
            date: true, // ← CHANGÉ (avant: createdAt)
          },
        },
        like: {
          select: {
    ...
    if (!pending.length) {
      console.log("[digest] rien à envoyer");
    ...
    for (const [destId, items] of byUser.entries()) {
      ...
      for (const v of visites) {
        sectionsHTML += `
          <tr><td style="padding:12px;border-bottom:1px solid rgba(255,255,255,0.1);">
      ...
      for (const [convId, convItems] of byConv.entries()) {
        ...
        const preview =
          last?.message?.type === "IMAGE" ? "(image)" :
          last?.message?.type === "AUDIO" ? "(audio)" :
          escapeHtml(excerpt(last?.message?.contenu || "(message)"));

        const maxLines = 3;
        const extra = Math.max(0, convItems.length - maxLines);
        const lines = convItems.slice(-maxLines).map(ci => {
          const who = ci?.message?.auteur?.pseudo || "Quelqu'un";
          const p =
            ci?.message?.type === "IMAGE" ? "(image)" :
            ci?.message?.type === "AUDIO" ? "(audio)" :
        ...
        sectionsHTML += `
          <tr><td style="padding:12px;border-bottom:1px solid rgba(255,255,255,0.1);">
            ...
            <div style="color:rgba(255,255,255,0.85);">Dernier : <strong>${escapeHtml(auteur)}</strong> — ${preview}</div>
        ...
        `<div style="background:#001f3f;padding:30px 0;">
          <div style="max-width:640px;margin:0 auto;background:rgba(255,255,255,0.05);border-radius:12px;overflow:hidden;">
            ...
            <div style="padding:20px;">
              <h2 style="font-weight:700;font-size:20px;margin-bottom:12px;color:#fff;">Bonjour ${escapeHtml(pseudo)},</h2>
              <p style="margin-bottom:20px;color:rgba(255,255,255,0.85);font-size:15px;">Voici vos dernières interactions :</p>
              <table style="width:100%;border-collapse:collapse;">${sectionsHTML}</table>
              <p style="margin-top:18px;color:rgba(255,255,255,0.6);font-size:12px;">Vous pouvez ajuster la fréquence de ces emails dans vos paramètres.</p>
            </div>
          </div>
        </div>`;
      ...
      try {
        if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM) {
          console.warn("[digest] RESEND désactivé (env manquantes), skip envoi");
        } else {
          await resend.emails.send({
            from: `"Xpérience" <${process.env.EMAIL_FROM}>`,
            to: email,
            subject: "📬 Votre récap’ — Xpérience",
            html,
            text,
        ...
        console.error(`[digest] erreur envoi user ${destId}:`, mailErr);
        const ids = items.map(x => x.id);
        await prisma.digestNotification.updateMany({
          where: { id: { in: ids } },
          data: { processingAt: null }, // libère pour retry
        });
      }
    }
    ...
    // ne laisse pas la cron « bloquer » les rows si crash
    await prisma.digestNotification.updateMany({
      where: { processingAt: { not: null }, sentAt: null },
      data: { processingAt: null },
    });
    return { ok: false, error: "Digest failed (see server logs)" };
  }
}
